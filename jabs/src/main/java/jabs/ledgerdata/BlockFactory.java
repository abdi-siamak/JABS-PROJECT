...
public final class BlockFactory {
    ...
    public static int sampleBitcoinCompactBlockSize(RandomnessEngine randomnessEngine) {
        return (int) randomnessEngine.sampleDistributionWithBins(BITCOIN_BLOCK_SIZE_2020, BITCOIN_COMPACT_BLOCK_SIZE_2020_BINS);
    }
    ...
    public static PAXOSBlock samplePAXOSBlock(Simulator simulator, RandomnessEngine randomnessEngine, PAXOSNode creator, PAXOSBlock parent) {
        return new PAXOSBlock(sampleBitcoinBlockSize(randomnessEngine), parent.getHeight() + 1,
                simulator.getSimulationTime(), creator, parent); // TODO: Size of PAXOS Blocks
    }
    
    public static RAFTBlock sampleRAFTBlock(Simulator simulator, RandomnessEngine randomnessEngine, RAFTNode creator, RAFTBlock parent) {
        return new RAFTBlock(sampleBitcoinBlockSize(randomnessEngine), parent.getHeight() + 1,
                simulator.getSimulationTime(), creator, parent); // TODO: Size of RAFT Blocks
    }

    public static BECPBlock sampleBECPBlock(Simulator simulator, RandomnessEngine randomnessEngine, BECPNode creator, int leader, BECPBlock parent, double VPropagation, double WPropagation, double VAgreement, double WAgreement, double VDataAggregation, double WDataAggregation, double VDataConvergence, double VDataAgreement, double WeightValue) {
    	return new BECPBlock(sampleBitcoinBlockSize(randomnessEngine), parent.getHeight() + 1,
                simulator.getSimulationTime(), creator.getCycleNumber() , creator, leader, parent, BECPBlock.State.PROPAGATION, VPropagation, WPropagation, VAgreement, WAgreement, VDataAggregation, WDataAggregation, VDataConvergence, VDataAgreement, WeightValue); // TODO: Size of BECP Blocks
    }
